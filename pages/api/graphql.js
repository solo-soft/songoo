...
const resolvers = {
    ...
    Mutation: {
        SET_TO_SUBSCRIBE_LIST: async (_, args) => {
            ...
            const {data, error} = await supabase.from("SUBSCRIBE_LIST").insert({
                id: artistsId,
                name,
                images,
                dependent_to,
        ...
        UserVideoSaved: async (_, args) => {
            const {userId, videoInfo , email} = args
            const {data, error} = await supabase.from("UserVideoSaved").insert({userId, videoInfo , email}).select("*")
            if (error) return error
            return data
        }
    }
...
const typeDefs = gql`
    ...
    input inputs_videoInfo {
        videoId: String,
        title: String,
        thumbnails: [inputs_url]
    }
    ...
    type Mutation {
        ...
        UserVideoSaved(userId : String , email : String , videoInfo : [inputs_videoInfo]) : [return_UserVideoSaved]
    }
`;

...
export default function Artist({ artist }: { artist: artist }) {
  ...
  const [artistName, setArtistsName] = useRecoilState<string | undefined>(
    ARTISTS_NAME
  );
  ...
  return (
    <Stack spacing={3} h={450}>
      <Button onClick={() => setOpenCollapse((prevState) => !prevState)}>
      ...
      {openCollapse && (
        <Stack position={"relative"}>
          ...
          >
            {singersUS.map((name) => (
              <Text
                onClick={(event) => handelChange(event.target.innerText)}
                key={Math.random()}
              >
                {name}
              </Text>
